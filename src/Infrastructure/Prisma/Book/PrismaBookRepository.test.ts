...
describe('PrismaBookRepository', () => {
  ...
  test('saveした集約がfindで取得できる', async () => {
    ...
    expect(createdEntity?.id.equals(bookId)).toBeTruthy();
  ...
  test('updateできる', async () => {
    ...
    const updatedEntity = await repository.find(createdEntity.id);
    expect(updatedEntity?.id.equals(book.id)).toBeTruthy();
  ...
  it('deleteできる', async () => {
    ...
    const readEntity = await repository.find(createdEntity.id);
    expect(readEntity).not.toBeNull();

    await repository.delete(createdEntity.id);
    const deletedEntity = await repository.find(createdEntity.id);
